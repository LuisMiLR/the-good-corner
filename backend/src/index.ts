...
app.use(express.json()); // Avec le middleware express.json() les données JSON envoyées par le client sont automatiquement converties en objets JavaScript le corps de requêtes JSON.. au lieu d'utiliser JSON.stringify(value)
...
app.post("/ads", async (req: Request, res: Response) => {
  ...
  try {
    ...
    await dataSource.manager.save(ad); //retourne entitymanager de la dataSource
